/**
  ******************************************************************************
  * @file    main.c
  * @author  https://github.com/niekiran/MasteringRTOS/tree/master/RTOS_workspace/STM32_FreeRTOS_Cnt_Sema_Tasks
  * @version V1.0
  * @date    2019/08/30
  * @brief   Synchronization by Binary semaphore (between tasks)
  ******************************************************************************
*/
...
int main( void ) {
    ...
    if( (xWork != NULL) && (xWorkQueue != NULL) )
    {
        ...
        xTaskCreate(
					...
					500,
					NULL,
					3,  // The Manager task is created with a high priority
					NULL
		);
...
static void vManagerTask( void *pvParameters ) {
	 ...
	 unsigned int xWorkTicketId; // consider as a sensor data
    ...
    while(1) {
        ...
        xWorkTicketId = ( rand() & 0x1FF ); // consider as a sensor data

		/* Sends work ticket id to the work queue */
		xStatus = xQueueSend( //Post an item on back of the queue
								xWorkQueue,
								&xWorkTicketId ,
								portMAX_DELAY
							);
...
static void vEmployeeTask( void *pvParameters )
{
	...
	unsigned char xWorkTicketId; // placeholder
	portBASE_TYPE xStatus;
	...
	while(1) {
		...
		else
		{
			...
			// This must be an error as this task should only run when the manager assigns at least one work. */
